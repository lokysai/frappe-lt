...
3. Confirm `sha256sum apps/frappe_lt/frappe_lt/release_catalog.json apps/frappe_lt/frappe_lt/locale/lt.po` against the pin and manifest respectively. `frappe_lt.release_catalog.verify_release()` is the read-only PO/key/context/digest check; `frappe_lt.release_catalog.verify_mo()` checks the bench MO once compiled. The MO is generated at `sites/assets/locale/lt/LC_MESSAGES/frappe_lt.mo` and is **bench-wide**, not a per-site file. CI should rebuild from the authenticated segments, byte-compare the shipped PO, compare a newly compiled MO digest, and test failures without replacing the active artifacts. An upstream Active Translation Key drift must block the deployment gate.
   ...
   The documented Frappe command for an **isolated release bench only** is `SOURCE_DATE_EPOCH=1704067200 bench compile-po-to-mo --app frappe_lt --locale lt --force`; check the new digest against the pinned full-release manifest. During site installation use the guarded hook instead of forcing a bench-wide MO replacement.
...
4. Check the existing bench-shared MO before `install-app`. If it already matches the release digest, reuse it. If absent, the hook compiles the **full shipped PO** in isolation and publishes the digest-matched MO **after** #13 SQL commit/report/cache recovery, while holding a bench-wide lock; it requires all sites to be prepared under the same release and in maintenance. A newly added/unprepared site or another site's incompatible pin blocks publication. The Compatibility MO digest is a smoke baseline; do not compare it to the full release MO. Never force a compile over an existing incompatible MO while another site uses it. If the hook reports `SHARED_MO_INCOMPATIBLE`, keep maintenance on, inspect all sites' release pins, coordinate a bench-wide MO change, then resume. Check the hook's output rather than recompiling it in place.

...
The committed catalog and candidate classifier are intentionally incomplete while [#11](https://github.com/lokysai/frappe-lt/issues/11), [#12](https://github.com/lokysai/frappe-lt/issues/12), and [#14](https://github.com/lokysai/frappe-lt/issues/14) own the remaining product translations and reviewed denominator. Runtime CI therefore expects exit `1` and a validated fail-closed report with English Fallback and Runtime Coverage Gap blockers. A green CI step means the harness correctly detected those known blockers, not that a release candidate passed; [#16](https://github.com/lokysai/frappe-lt/issues/16) owns final release validation after the product-data issues close.
